      ...
      >
        ...
        >
          ...
          <div
            ...
            v-if="(showToggleAll && selectionLimit == null) || filter"
          >
            <div
              class="p-checkbox p-component alguma-classe-que-quero-ver"
              id="digaNaoAcessibilidade"
              @click="onToggleAll"
              role="checkbox"
            ...
            >
              ...
              >
                <span
                  :class="['p-checkbox-icon', { 'pi pi-check': allSelected }]"
                ></span>
              </div>
            </div>
...
export default {
  ...
  beforeUnmount() {
    ...
    if (this.$refs.overlayPanel) {
      ZIndexUtils.clear(this.$refs.overlayPanel);
      this.$refs.overlayPanel = null;
    }
  },
  methods: {
    focusedPanel() {
      setTimeout(() => {
        console.log("deu bom mamae");
        document.getElementById("digaNaoAcessibilidade").focus();
      }, 1001);
    },
      ...
      if (this.modelValue != null && this.options) {
        if (this.optionGroupLabel) {
          for (let i = 0; i < this.options.length; i++) {
            let selectedOptionIndex = this.findOptionIndexInList(
              this.modelValue,
              this.getOptionGroupChildren(this.options[i])
            );
    ...
    isSelected(option) {
      if (this.modelValue) {
        ...
        return this.modelValue.some((val) =>
          ObjectUtils.equals(val, optionValue, key)
        );
      }
      return false;
    },
    ...
    onClick(event) {
      ...
      if (
        (!this.$refs.overlayPanel ||
          !this.$refs.overlayPanel.contains(event.target)) &&
    ...
    alignOverlay() {
      if (this.appendDisabled) {
        DomHandler.relativePosition(this.$refs.overlayPanel, this.$el);
      } else {
        this.$refs.overlayPanel.style.minWidth =
          DomHandler.getOuterWidth(this.$el) + "px";
        DomHandler.absolutePosition(this.$refs.overlayPanel, this.$el);
      }
    },
      ...
      if (this.options) {
        if (this.optionGroupLabel) {
          for (let optionGroup of this.options) {
            option = this.findOptionByValue(
              val,
              this.getOptionGroupChildren(optionGroup)
            );
            if (option) {
              break;
            }
      ...
      if (this.selectAll !== null) {
        ...
        if (this.allSelected) {
          ...
          if (this.optionGroupLabel) {
            ...
            value = this.visibleOptions.map((option) =>
              this.getOptionValue(option)
            );
          }
        }
    ...
    removeChip(item) {
      let value = this.modelValue.filter(
        (val) => !ObjectUtils.equals(val, item, this.equalityKey)
      );
      this.$emit("update:modelValue", value);
      this.$emit("change", { originalEvent: event, value: value });
    },
          ...
          if (this.modelValue && this.options) {
            ...
            if (this.optionGroupLabel)
              this.options.forEach(
                (optionGroup) =>
                  (optionCount +=
                    this.getOptionGroupChildren(optionGroup).length)
              );
            else optionCount = this.options.length;
